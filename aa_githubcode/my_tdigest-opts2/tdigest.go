...
func (d *TDigest) Dump() string {
	//for _, v:=range d.centroids{
	//	log.Print(*v)
	//}
	//log.Println("")
	return fmt.Sprintf("len(centroids)=%d, countTotal=%d", len(d.centroids), d.countTotal)
}
...
func (d *TDigest) nearest(val float64) []int {
	var (
		...
		//result []int = make([]int, 10)
	)

	lenDeltaEqual0 := 0 //todo
